...
//                 className="w-full bg-blue-500 text-white py-2 rounded hover:bg-blue-600 mb-4 cursor-pointer"
//               >
//                 Edit Profile
//               </button>
//             ) : (
...
//                 className="w-full bg-green-500 text-white py-2 rounded hover:bg-green-600 mb-4 cursor-pointer"
//               >
//                 Save Changes
//               </button>
//             )}
...
//               className="w-full bg-red-500 text-white py-2 rounded hover:bg-red-600 mt-4 cursor-pointer"
//             >
//               Sign Out
//             </button>
//           </>
...
export default function Profile() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        const userDoc = await getDoc(userDocRef);
        if (userDoc.exists()) {
          setRideStats(userDoc.data().rideStats || {});
        } else {
          setRideStats({ rides: 0, distance: 0, ratings: 0 });
        }
      } else {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      // Update local state and revert to non-editing mode
      setUser((prev) => ({ ...prev, displayName: updatedName }));
      setEditing(false); // Ensure the button reverts to "Edit Profile"
      alert("Profile updated successfully!");
    } catch (error) {
      console.error("Error updating profile:", error);
      alert("Failed to update profile. Please try again.");
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-md">
        {user && (
          <>
            ...
            {/* Buttons: Edit Profile / Save Changes */}
            <button
              onClick={editing ? handleUpdateProfile : () => setEditing(true)}
              className={`w-full py-2 rounded mb-4 cursor-pointer ${
                editing

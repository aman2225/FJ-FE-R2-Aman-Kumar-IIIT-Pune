...
const Search = () => {
  ...
  const geocodingClient = mbxGeocoding({
    accessToken: "pk.eyJ1IjoiYW1hbjIyMjUiLCJhIjoiY203ZzA0Z3BlMHU3djJrc2V2c2lhdGd4ciJ9.jqojL31H4cghFWXayhsdCA",
  });
      ...
      <div className="flex bg-white w-full px-4 py-4 relative items-center">
        ...
        <div className="flex flex-col flex-1 pr-14">
          <div className="mb-3 relative">
            ...
            />
            {pickupSuggestions.length > 0 && (
              <ul className="absolute top-full left-0 bg-white shadow-md w-full z-10 max-h-48 overflow-auto">
                {pickupSuggestions.map((suggestion, index) => (
                  <li
                    key={index}
                    onClick={() => handleSuggestionClick("pickup", suggestion)}
                    className="cursor-pointer px-4 py-2 hover:bg-gray-100 text-black truncate"
                  >
                    {suggestion.placeName}
                  </li>
                ))}
